...
public class ObjectsServiceMongoDb implements ObjectServiceWithPaginationSupport {
    ...
    public SuperAppObjectBoundary createObject(SuperAppObjectBoundary object) {
        ...
        logger.trace("Passed the boundaryToEntity in create an object and the SuperAppObjectEntity is: " + superAppObjectEntity);
        ...
        logger.trace("Passed the setCreationTimestamp and setObjectId in create an object and the SuperAppObjectEntity is: " + superAppObjectEntity);

        superAppObjectEntity = this.databaseCrud.save(superAppObjectEntity);
        logger.trace("Passed the save to mongodb in create an object and the SuperAppObjectEntity is: " + superAppObjectEntity);
    ...
    private void checkObjectValidity(SuperAppObjectBoundary object) {
        ...
        if (lat == null || lat < Math.negateExact(Constants.LATITUDE_RANGE) || lat > Constants.LATITUDE_RANGE) {
            logger.warn("Latitude value in Location object is null or not in range(-" + Constants.LATITUDE_RANGE + " <-> " + Constants.LATITUDE_RANGE + ")");
            throw new SuperAppObjectBadRequestException(
                    "Latitude value in Location object is null or not in range(-" + Constants.LATITUDE_RANGE + " <-> " + Constants.LATITUDE_RANGE + ")");
        }

        if (lng == null || lng < Math.negateExact(Constants.LONGITUDE_RANGE) || lng > Constants.LONGITUDE_RANGE) {
            logger.warn("Longitude value in Location object is null or not in range(-" + Constants.LONGITUDE_RANGE + " <-> " + Constants.LONGITUDE_RANGE + ")");
            throw new SuperAppObjectBadRequestException(
                    "Longitude value in Location object is null or not in range(-" + Constants.LONGITUDE_RANGE + " <-> " + Constants.LONGITUDE_RANGE + ")");
        }
    ...
    public SuperAppObjectBoundary updateAnObject(String objectSuperApp, String internalObjectId,
        ...
        if (user.getRole() == UserRole.SUPERAPP_USER) {
            ...
            logger.trace("Passed the searching object in update an object and the SuperAppObjectEntity is: " + existingObject);
            boolean dirtyFlag = false;
            if (update.getActive() != null) {
                logger.trace("Passed the update active in update an object and the update active is: " + update.getActive());
            ...
            if (update.getAlias() != null && !update.getAlias().isEmpty()) {
                logger.trace("Passed the update alias in update an object and the update alias is: " + update.getAlias());
            ...
            if (update.getLocation() != null) {
                logger.trace("Passed the update location in update an object and the update location is: " + update.getLocation());
            ...
            if (update.getObjectDetails() != null) {
                logger.trace("Passed the update objectDetails in update an object and the update objectDetails is: " + update.getObjectDetails());
    ...
    public Optional<SuperAppObjectBoundary> getSpecificObject(String objectSuperApp, String internalObjectId,
        ...
        logger.trace("Entering to the method getSpecificObject with the parameters: " + objectSuperApp + " " + internalObjectId + " " + userSuperapp + " " + userEmail);
        String attr = objectSuperApp + Constants.DELIMITER + internalObjectId;
        String userId = userSuperapp + Constants.DELIMITER + userEmail;
        UserEntity user = this.userCrud.findById(userId)
                .orElseThrow(() -> new UserNotFoundException(userNotFoundByIdMessage + userId));
        ...
        logger.trace("Passed the searching object in getSpecificObject and the SuperAppObjectEntity is: " + superAppObjectEntity);
        if (user.getRole() == UserRole.SUPERAPP_USER) {
    ...
    public List<SuperAppObjectBoundary> getAllObjects(String userSuperapp, String userEmail, int size, int page) {
        logger.trace("Entering to the method getAllObjects with the parameters: " + userSuperapp + " " + userEmail + " " + size + " " + page);
        String userId = userSuperapp + Constants.DELIMITER + userEmail;
        UserEntity user = this.userCrud.findById(userId)
                .orElseThrow(() -> new UserNotFoundException(userNotFoundByIdMessage + userId));
    ...
    public void BindAnExistingObjectToExistingChildObject(String superapp, String internalObjectId,
        ...
        logger.trace("Entering to the method BindAnExistingObjectToExistingChildObject with the parameters: " + superapp + " " + internalObjectId + " " + childId + " " + userSuperapp + " " + userEmail);
        String userId = userSuperapp + Constants.DELIMITER + userEmail;
        UserEntity user = this.userCrud.findById(userId)
                .orElseThrow(() -> new UserNotFoundException(userNotFoundByIdMessage + userId));
        logger.trace("Passed the searching user in BindAnExistingObjectToExistingChildObject and the UserEntity is: " + user);
        if (user.getRole() == UserRole.SUPERAPP_USER) {
    ...
    public List<SuperAppObjectBoundary> getAllChildrenOfAnExistingObject(String superapp, String internalObjectId,
        ...
        logger.trace("Entering to the method getAllChildrenOfAnExistingObject with the parameters: " + superapp + " " + internalObjectId + " " + userSuperapp + " " + userEmail + " " + size + " " + page);
        String userId = userSuperapp + Constants.DELIMITER + userEmail;
    ...
    public List<SuperAppObjectBoundary> getAnArrayWithObjectParent(String superapp, String internalObjectId,
                                                                   String userSuperapp, String userEmail, int size, int page) {
        logger.trace("Entering to the method getAnArrayWithObjectParent with the parameters: " + superapp + " " + internalObjectId + " " + userSuperapp + " " + userEmail + " " + size + " " + page);
        String userId = userSuperapp + Constants.DELIMITER + userEmail;
        String attr = superapp + Constants.DELIMITER + internalObjectId;
        UserEntity user = this.userCrud.findById(userId)
                .orElseThrow(() -> new UserNotFoundException(userNotFoundByIdMessage + userId));
    ...
    public List<SuperAppObjectBoundary> searchObjectsByType(String superapp, String email, String type, int size,
                                                            int page) {
        logger.trace("Entering to the method searchObjectsByType with the parameters: " + superapp + " " + email + " " + type + " " + size + " " + page);
    ...
    public List<SuperAppObjectBoundary> searchObjectsByAlias(String superapp, String email, String alias, int size,
                                                             int page) {
        logger.trace("Entering to the method searchObjectsByAlias with the parameters: " + superapp + " " + email + " " + alias + " " + size + " " + page);
        String userId = superapp + Constants.DELIMITER + email;
        UserEntity user = this.userCrud.findById(userId)
                .orElseThrow(() -> new UserNotFoundException(userNotFoundByIdMessage + userId));
        logger.trace("Passed the searching user and the UserEntity is: " + user);
    ...
    public List<SuperAppObjectBoundary> searchObjectsByLocation(String superapp, String email, double lat, double lng,
        ...
        logger.trace("Entering to the method searchObjectsByLocation with the parameters: " + superapp + " " + email + " " + lat + " " + lng + " " + distance + " " + units + " " + size + " " + page);
        String userId = superapp + Constants.DELIMITER + email;
        ...
        logger.trace("Passed the distance calculation and the minLat is: " + minLat + " maxLat is: " + maxLat + " minLng is: " + minLng + " maxLng is: " + maxLng);
        UserEntity user = this.userCrud.findById(userId)
                .orElseThrow(() -> new UserNotFoundException(userNotFoundByIdMessage + userId));
        logger.trace("Passed the searching user and the UserEntity is: " + user);
        if (user.getRole() == UserRole.SUPERAPP_USER) {
    ...
    public List<SuperAppObjectBoundary> searchObjectsByLocationCircle(String superapp, String email, double lat,
        ...
        logger.trace("Entering to the method searchObjectsByLocationCircle with the parameters: " + superapp + " " + email + " " + lat + " " + lng + " " + distance + " " + units + " " + size + " " + page);
        String userId = superapp + Constants.DELIMITER + email;
        UserEntity user = this.userCrud.findById(userId)
                .orElseThrow(() -> new UserNotFoundException(userNotFoundByIdMessage + userId));
        logger.trace("Passed the searching user and the UserEntity is: " + user);
